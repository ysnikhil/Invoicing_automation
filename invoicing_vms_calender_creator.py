...
df.loc[(df['vms_working_days'] == df['leave_working_days']) &  (df['weekend_flag'] == 1) & (df['leave_hours'] == 0) & (df['leave_working_wkenddays'] == 0) & (df['vms_pending_hours'] > 0), 'final_output'] = 0
df['vms_pending_hours'] = df['vms_hours'] - df.groupby('vms_week').final_output.transform('sum')
